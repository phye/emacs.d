...
(defun
 ...
 (interactive)
 (with-temp-buffer (insert val) (mark-whole-buffer) (clipetty-kill-ring-save))
 (message "copied: %s" val))

(defun
 copy-relative-dir-in-project () (interactive)
 (let ((path (file-name-directory (file-relative-name (buffer-file-name) (ffip-project-root)))))
   (copy-variable-to-clipboard path)))

(defun
 copy-relative-path-in-project () (interactive)
 (let ((path (file-relative-name (buffer-file-name) (ffip-project-root))))
   (copy-variable-to-clipboard path)))

(defun
 copy-project-root-to-clipboard () (interactive) (copy-variable-to-clipboard (ffip-project-root)))

(defun copy-full-dir-to-clipboard () (interactive) (copy-variable-to-clipboard default-directory))
...
(defun insert-tab () "Insert tab." (interactive) (insert-char ?\t))
