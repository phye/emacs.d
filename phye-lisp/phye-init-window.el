...
(when (boundp 'fringe-mode) (fringe-mode 0))
(setq switch-to-buffer-in-dedicated-window 'pop)
(setq switch-to-buffer-obey-display-actions t)
(setq window-sides-slots '(1 1 1 1))

(with-eval-after-load
 'ace-window (customize-set-variable 'aw-scope 'frame) (customize-set-variable 'aw-background t))
...
(defun
 phye/kill-buffer-and-frame () "Kill buffer and frame." (interactive) (kill-buffer) (delete-frame))

(defun
 phye/open-recent-file-in-other-frame () "Open recent file in other frame." (interactive)
...
(defun
 ...
 (interactive "p")
 (if
  phye--center-window-maxmized
  (progn (message "Already maximized, restore...") (balance-windows))
  (when (< width window-min-width) (setq width 60))
  ...
  (let* ((windows (window-list nil 0 (frame-first-window)))
    ...
    (dolist
     (w windows)
     (unless (eq w center-window) (with-selected-window w (evil-window-set-width width))))))
...
(require 'cl-lib)
(defun
 phye/kill-last-opened-buffer () "Kill last opened buffer except current." (interactive)
 (cl-dolist
  (buf (buffer-list (selected-frame)))
  (when
   (and (not (eq buf (current-buffer))) (not (minibufferp buf)))
...
(defun
 phye/scroll-window-left (&optional lines) "Scroll window left" (interactive "P")
 (with-selected-window
  (window-left (selected-window))
  (funcall (or (command-remapping #'scroll-up-command) #'scroll-up-command) lines)))

(defun
 phye/scroll-window-right (&optional lines) "Scroll window left" (interactive "P")
 (with-selected-window
  (window-right (selected-window))
  (funcall (or (command-remapping #'scroll-up-command) #'scroll-up-command) lines)))

(defun phye/xref-got-xref-and-quit () (interactive) (xref-goto-xref t))
...
(defun
 phye/mv-current-buffer-to-window (window) "Move current buffer to WINDOW."
 (let* ((cur-buffer (current-buffer))
        (cur-window (get-buffer-window cur-buffer)))
   (with-selected-window cur-window (switch-to-prev-buffer cur-window))
