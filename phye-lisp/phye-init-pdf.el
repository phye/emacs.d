...
(defun
 phye/pdf--goto-page-by-cond (condfunc) "Go to title page by CONDFUNC in pdf."
 (let* ((buffer (current-buffer))
   ...
   (cl-loop
    ...
    (let* ((title-item-0 (nth 0 toc))
      ...
      (if
       (>= target-page 0)
       (progn (setq found t) (pdf-view-goto-page target-page window))
       (setq toc (cdr toc)))))))

(defun
 phye/pdf-goto-next-title-page () "Go to next title page." (interactive)
 (phye/pdf--goto-page-by-cond
  (lambda
   (cur-page title-page-0 title-page-1)
   (if
    (and (<= title-page-0 cur-page) (< cur-page title-page-1))
    (progn
     ;; (message "found")
     title-page-1)
    -1))))
...
(defun
 phye/pdf-goto-prev-title-page () "Go to next title page." (interactive)
 (phye/pdf--goto-page-by-cond
  (lambda
   (cur-page title-page-0 title-page-1)
   (if
