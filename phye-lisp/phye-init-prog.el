...
(setq
 phye/general-ignore-directories
...
(defun
 phye/ediff-startup-hook
 ()
 (setq previous-theme (car custom-enabled-themes))
 (load-theme 'doom-gruvbox t))
(defun phye/ediff-cleanup-hook () (load-theme previous-theme t) (ediff-janitor nil t))
(add-hook 'ediff-startup-hook #'phye/ediff-startup-hook)
(add-hook 'ediff-cleanup-hook #'phye/ediff-cleanup-hook)

;; log
(defun
 phye/view-log-with-color () (interactive) (ansi-color-apply-on-region (point-min) (point-max)))
...
;; python
(with-eval-after-load 'eldoc-mode (setq eldoc-idle-delay 5))

(defun
 phye/python-mode-hook
 ()
...
(defun
 phye/goto-definition-at-point () "my mode-aware go to definition" (interactive)
 (cl-case
  major-mode
...
(defun
 phye/format-buffer () "My format buffer wrapper." (interactive)
 (cl-case
  major-mode
  (go-mode (gofmt))
...
(defun
 phye/xref-jump-to-only-reference
 (IDENTIFIER)
 ...
 (let ((xref-buffer (get-buffer "*xref*")))
   (with-current-buffer
    ...
    (when
     (eq (count-lines (point-min) (point-max)) 2)
     (xref-next-line)
     (xref-goto-xref)
...
(defun
 phye/get-project-name-of-active-window () "Return project name of active window."
 (let* ((project-root (ffip-project-root))
        ...
        (shortname (if (>= len 10) (substring project-name 0 10) project-name)))
   shortname))

(defun
 phye/set-tmux-window-based-on-project
 (&optional window)
 "Set tmux window name to current active project, WINDOW is not used yet."
 (interactive)
 (when
  (and (not (display-graphic-p)) (not (string-empty-p (getenv "TMUX"))))
