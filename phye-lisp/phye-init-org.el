...
(setq
 ...
 '(("t" "Todo" entry (file+headline "~/ws/gtd/gtd.org" "Tasks") "* TODO %?\n %i\n %a")
...
(setq org-babel-default-header-args:plantuml '((:results . "file") (:exports . "results")))
(setq org-babel-default-header-args:icalendar '((:exports . "none")))
...
(setq
 org-tag-alist
 '((:startgroup . nil) ;; tag group for address
   ("@work" . ?w)
   ("@home" . ?h)
   ("@travel" . ?t)
   (:endgroup . nil)
...
(with-eval-after-load
 ...
 (customize-set-variable
  'calendar-holidays
  (append holiday-oriental-holidays holiday-local-holidays holiday-other-holidays)))
(with-eval-after-load
 'ox-icalendar
...
(use-package
 ...
 :config (customize-set-variable 'org-journal-date-format "%A, %x"))
;; org-journal capture
...
(defun
 ...
 (interactive) (delete-file (buffer-name)) (kill-buffer))
;; }}
...
(use-package
 ...
 :ensure t
 :defer t
 :bind (:map org-noter-notes-mode-map ("C-x o" . other-window))
 :config (pdf-tools-install))
;; }}

;; {{ org-side-tree
(use-package org-side-tree :ensure t :defer t)
;; }}
...
(defun
 phye/org-remark-notes-file-name () "Return notes file name as <project>-notes.org."
...
(use-package
 ...
 :custom (org-fancy-priorities-list '((?A . "P0") (?B . "P1") (?C . "P2"))))
;; }}
...
(defvar
 ...
 (rx-to-string
  `(seq
    (group-n 1 (category chinese))
    (group-n 2 (any "~+_*"))
    (group-n 3 (or (any alnum) (any "-/{}")))))
...
(defvar
 ...
 (rx-to-string
  `(seq
    (group-n 1 (or (any alnum) (any "-/{}")))
    (group-n 2 (any "~+_*"))
    (group-n 3 (category chinese))))
 "Regexp for matching english-mark-chinese.")

(defun
 phye/remove-zws-in-region
 (begin end)
...
(defvar
 phye/in-word-white-spaces-regex "\\(\\cc\\) +\\(\\cc\\)" "Regexp for in word white spaces.")
...
(defun
 phye/cleanup-white-spaces-in-string
 (str)
...
;; {{ hooks
(defun
 phye/org-mode-hook () "custom orgmode settings" (interactive)
...
(defun
 phye/org-icalendar-after-save-hook
 (file)
...
(with-eval-after-load
 ...
 (my-run-with-idle-timer
  ...
  (lambda
   ()
   ;; org babel
   (org-clock-persistence-insinuate) (add-to-list 'org-src-lang-modes '("plantuml" . plantuml))
   (org-babel-do-load-languages
    'org-babel-load-languages
    '((mermaid .t) (ditaa . t) (plantuml . t) (dot . t) (emacs-lisp . t) (gnuplot . t)))
   (require 'graphviz-dot-mode) (require 'ox-md nil t) (require 'ox-odt nil t)
   ;; (require 'calfw-org)
   (require 'yasnippet) (my-yas-reload-all))))

(defun
 phye/org-before-save-hook () "phye's orgmode before save hook."
 (when
  (member major-mode '(org-mode markdown-mode gfm-mode))
  (when (eq major-mode 'org-mode) (phye/insert-zws-in-buffer))
  (phye/cleanup-white-spaces-in-buffer)))
(add-hook 'before-save-hook #'phye/org-before-save-hook)

;; TODO(phye): move this priority logic to md-after-export-hook
(defun
 phye/replace-priority-in-string (str) "Replace #[A|B|C] priority with P[0|1|2] in STR."
 (replace-regexp-in-string
  "#A" "P0" (replace-regexp-in-string "#B" "P1" (replace-regexp-in-string "#C" "P2" str))))

(defun
 phye/md-after-export-hook
 (text backend info)
